          ...
          <ScrollSpyList className="sticky top-32 w-[12vw] self-start">
            <div className="flex flex-col gap-5 lxl:gap-10">
              {items.map((item, index) => {
                ...
                return (
                  <ScrollSpyTrigger
                    className="relative w-full cursor-pointer border-transparent border-l-4 px-4 py-3 text-center text-body-subtle uppercase transition-all hover:text-title hover:underline"
                    key={tabId}
                    value={tabId}
                  >
                    {item.label}
